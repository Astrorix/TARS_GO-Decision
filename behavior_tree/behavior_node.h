...
enum class BehaviorType {
  PARALLEL,       ///<Parallel Composite Node  平行复合节点
  SELECTOR,       ///<Selector Composite Node 选择节点
  SEQUENCE,       ///<Sequence Composite Node 序列节点
  ACTION,         ///<Action Node 动作节点
  PRECONDITION,   ///<Precondition Node 前置条件节点
};
/**
...
class BehaviorNode : public std::enable_shared_from_this<BehaviorNode>{
 public:
  ...
  virtual void Reset(){
    if (behavior_state_ == BehaviorState::RUNNING){
      behavior_state_ = BehaviorState::IDLE;
      OnTerminate(behavior_state_);    //未完成
    }
  }
  /**
